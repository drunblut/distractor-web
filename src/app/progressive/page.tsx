...
function TaskQueueApp() {
  ...
  const getCurrentView = () => {
    // If we're in start or mood phase, show those screens
    if (appPhase === 'start') return 'start';
    if (appPhase === 'mood') return 'mood';
    
    // Once we're in task phase, use task queue
    if (appPhase === 'tasks' && currentTask) {
      ...
      case 'face2Recall': return 'face2-recall';        case 'handTask': return 'hand-task';
        ...
        default: return 'pie-task'; // Fallback to first task
      }
    }
  ...
  const handleMoodContinue = async (data: { moodValue: number; mainSymptom: string; symptomIntensity: number }) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        console.log('[ProgressiveApp] Session created:', sessionData);
        
        // Navigate to tasks
        setAppPhase('tasks');
      } else {
        console.error('[ProgressiveApp] Failed to create session:', result.error);
        // Still proceed to tasks even if database fails
        setAppPhase('tasks');
      }
    } catch (error) {
      console.error('[ProgressiveApp] Error creating session:', error);
      // Still proceed to tasks even if database fails
      setAppPhase('tasks');
    }
  };
  ...
  const handlePostGameMoodComplete = (moodNachher: number, symptomIntensityNachher: number) => {
    console.log('[ProgressiveApp] PostGameMoodScreen completed with mood:', moodNachher, 'symptom intensity:', symptomIntensityNachher);
